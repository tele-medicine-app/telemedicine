...
function initializePeer() {
  ...
  // Auto-discover the PeerJS signaling server relative to wherever
  // this page is loaded from. Works identically on:
  //   - localhost (dev)            -> ws + explicit port
  //   - https://*.onrender.com     -> wss + port 443
  //   - any other HTTPS host        -> wss + port 443
  // The backend (server.js) serves both this page AND /peerjs, so
  // the signaling endpoint is always same-origin. Crucially, when
  // deployed to HTTPS the browser hides port 443 in location.port
  // (returns ""), so we must NOT fall back to 3000 there.
